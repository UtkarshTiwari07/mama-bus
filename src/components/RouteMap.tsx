...
// Static mini-map for a single route page: the line draws itself, then a car drives it.
export function RouteMap({ from, to }: { from: CityKey; to: CityKey }) {
  ...
  return (
    <svg viewBox={`0 0 ${map.width} ${map.height}`} className="h-auto w-full" role="img" aria-label={`${a.hi} से ${b.hi} / ${a.name} to ${b.name}`}>
      {map.districts.map((dist) => (
        <path key={dist.name} d={dist.d} className="fill-ink/[0.06]" stroke="rgb(22 48 42 / 0.25)" strokeWidth="0.8" />
      ))}
      <path d={d} fill="none" stroke="#b3362b" strokeWidth="3.5" strokeLinecap="round" data-draw="" />
      <path d={d} fill="none" stroke="#b3362b" strokeWidth="3.5" strokeLinecap="round" strokeDasharray="1 10" opacity="0.35" />
      {[a, b].map((c, i) => (
        <g key={c.name}>
